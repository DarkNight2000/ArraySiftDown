...
public final class ArraySiftDownMain {
    ...
    private static void siftDown(Array<Integer> array, int top, int last) {
        ...
        int root = top;
        if (array.length() > 1) {
            if (left < last && array.entry(left) < array.entry(root)) {
                array.exchangeEntries(left, root);
                root = left;
            }
            if (right < last && array.entry(right) < array.entry(top)) {
                array.exchangeEntries(right, root);
                root = right;
            }
            if (root != top) {
